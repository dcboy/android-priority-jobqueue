...
public class SqliteJobQueue implements JobQueue {
  ...
  private void bindValues(SQLiteStatement stmt, JobHolder jobHolder) {
    if (jobHolder.getInsertionOrder() != null) {
      stmt.bindLong(DbOpenHelper.INSERTION_ORDER_COLUMN.columnIndex + 1,
          jobHolder.getInsertionOrder());
    }
    ...
    stmt.bindLong(DbOpenHelper.RUNNING_SESSION_ID_COLUMN.columnIndex + 1,
        jobHolder.getRunningSessionId());
  ...
  /**
   * This method is called when a job is pulled to run. It is properly marked so that it won't be
   * returned from next job queries.
   * <p/>
